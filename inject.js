            ...
            pc.getStats()
                .then(function (stats) {
                    ...
                    stats.forEach(function (v, k) {
                        let insterestedStats = []
                        let stat = {}
                        if (v.type === "media-source") {
                            insterestedStats = ["width", "height", "framesPerSecond"]
                        } else if (v.type === "inbound-rtp" && v.kind === "audio") {
                            insterestedStats = ["packetsReceived", "packetsLost", "jitter", "framesDropped"]
                        } else if (v.type === "inbound-rtp" && v.kind === "video") {
                            insterestedStats = ["framesDecoded", "framesReceived", "bytesReceived"]
                        } else if (v.type === "remote-inbound-rtp") {
                            insterestedStats = ["roundTripTime"]
                        }

                        if (insterestedStats.length > 0) {
                            Object.keys(v).forEach(function (key) {
                                if (insterestedStats.includes(key)) {
                                    stat[key] = v[key];
                                }
                            });
                            cpy[k] = stat;
